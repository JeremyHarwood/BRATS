...
#include "smoothmaps.h"
  ...
  if ( (exportfits !=1) || (export != 1) ) { // Do the standard plotting
    ...
    else if (errorpm == 0) { sprintf(specmaptitle, "Positive Error Map of %s", settarget); }
    else { sprintf(specmaptitle, "Negative Error Map of %s", settarget); }
    ...
    // Set plot title
    if (titles == 0) { sprintf(specmaptitle, " "); }
    else { sprintf(specmaptitle, "Error Map of %s", settarget); }
	...
	for (j=0; j<jsize; j++) {
	  if ( ((i+jstart-border) < 0) || ((j+istart-border) < 0) || ((i+jstart-border) >= xaxismax) || ((j+istart-border) >= yaxismax) ) {
	    ...
	    tmp_smootharray[i*jsize+j] = ageerrors[regionarray[i+(jstart-border)][j+(istart-border)]];
      ...
      if (adj_maxrange < 0.01) { // this prevents a 0 range, especially when using one region
	printf("\n*** Warning: Range is too small, setting maxrange to a value of 0.01\n\n"); 
	adj_maxrange = 0.01;
      }

      cpgctab(rl,rr,rg,rb,11,contra,bright);
      if (largetxt == 1) { cpgsch(1.30); }
      cpgwedg("RI", 1.5, 5.0, minrange, adj_maxrange, "Error (Myr)");
	...
	for (j=jstart; j<jend; j++){
	  if (regionarray[i][j] <= 0) {
	    ...
	    switch ((int)ageerrors[regionarray[i][j]]) {
	    case 0:
	      intenscale = 1;
	      break;
	    case 1:
	...
	if (regionarray[i][j] < 0) { // Set void regions to a high value to create a white background
	  ...
	  tmp_smootharray[i*xaxismax+j] = ageerrors[regionarray[i][j]];
    ...
    FunParamPuts(funconn, "BUNIT", 0,  "Myr", "Units of error", 1);
    ...
    FunParamPuts(funconn, "HISTORY", 0, historytext, NULL, 1);
    strcpy(historytext, "BRATS  ");
    if (errorpm == 0) {
      strcat(historytext, "Positive");
    }
    else {
      strcat(historytext, "Negative");
    }
    strcat(historytext, " error map for the ");
