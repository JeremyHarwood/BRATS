// Plots a spectral age map - If the age range is greater than 15, it is best to use smooth maps
...
#include "smoothmaps.h"
 ...
 // Set up variables and structs for times and dates
  ...
  if ( (exportfits !=1) || (export != 1) ) { // Do the standard plotting
    ...
    else { sprintf(specmaptitle, "Spectral Ageing Map of %s", settarget); }
	...
	minrange = 0;
      ...
      // Test code for new method, just set the whole array to the blank value to allow an aspect ratio
      /* for (i=0; i<isize; i++) { */
      /* 	for (j=0; j<jsize; j++) { */
      ...
      /* for (i=0; i<isize; i++) { */
      /* 	for (j=0; j<jsize; j++) { */
      /* 	  if ( ((i+jstart-border) >= 0) && ((j+istart-border) >= 0) && ((i+jstart-border) < xaxismax) && ((j+istart-border) < yaxismax) && (regionarray[i+(jstart-border)][j+(istart-border)] >= 0) ) { */

      /* 	      tmp_smootharray[i*jsize+j] = bestage[regionarray[i+(jstart-border)][j+(istart-border)]]; */
      /* 	      //printf("t_sa: %.3f\n", tmp_smootharray[i*jsize+j]); */
      /* 	      printf("i: %d j: %d\n", i, j); */
      /* 	      if ( (tmp_smootharray[i*jsize+j] < minrange) && (alwayszeroage != 1) ) { */
      ...
      /* for (i=0; i<isize; i++) { */
      /* 	for (j=0; j<jsize; j++) { */
      /* 	  if (tmp_smootharray[i*jsize+j] < 1e20) { */
      /* 	    printf("t: %d j: %d t_sa: %.3f\n", i, j, tmp_smootharray[i*jsize+j]); */
      /* 	  } */
      /* 	} */
      /* } */
	...
	adj_maxrange = 0.1;
      ...
      // Maybe at a later date if I have time
      /* int pubformat = 1; */
      /* int wedgepos = 1; // 1 = top, 2 = right */

      /* if (pubformat == 1) { */
      /* 	if (wedgepos == 1) { */
      /* 	  cpgwedg("TI", 0, 5.0, minrange, adj_maxrange, "Spectral Age (Myr)"); */
      /* 	} */
      /* 	else if (wedgepos == 2) { */
      /* 	  cpgwedg("RI", 0, 5.0, minrange, adj_maxrange, "Spectral Age (Myr)"); */
      ...
      if (largetxt == 1) {
	cpgsch(1.30);
	cpgwedg("RI", 1.5, 5.0, minrange, adj_maxrange, "Spectral Age (Myr)");
      }
      else {
	cpgwedg("RI", 1.5, 5.0, minrange, adj_maxrange, "Spectral Age (Myr)");
      }
	...
	for (j=jstart; j<jend; j++){
	  if (regionarray[i][j] <= 0) {
	    ...
	    switch ((int)bestage[regionarray[i][j]]) {
	    case 0:
	      intenscale = 1;
	      break;
	    case 1:
    ...
    if (plotbeam == 1) {
      ...
      // printf("beamposi: %d iend: %d border: %d bmaj: %.3f cellsize: %.3f beamposx: %d istart: %d isize: %d lhs: %.3f rhs: %.3f tmp_beamposx: %.3f \n", beamposi,iend,border,bmaj,cellsize,beamposx,istart,isize, iend+border-(bmaj/cellsize),tmp_beamposx*isize, tmp_beamposx);
	...
	for (j=0; j<yaxismax; j++) {
	  ...
	  if (regionarray[i][j] < 0) { // Set void regions to a high value to create a white background
	    ...
	    tmp_smootharray[i*xaxismax+j] = bestage[regionarray[i][j]];

	    if ( (tmp_smootharray[i*xaxismax+j] < minrange)  && (alwayszeroage != 1) ) {
	...
	return 404;
      ...
      FunParamPuts(funconn, "BUNIT", 0,  "MYr", "Units of spectral age", 1);
	...
	strcpy(modelname, "Tribble (KP)");
      ...
      FunParamPuts(funconn, "HISTORY", 0, historytext, NULL, 1);
      strcpy(historytext, "BRATS  Spectral ageing map made using the ");
