// Plots an injection index by region map
...
#include "smoothmaps.h"
 ...
 // Set up variables and structs for times and dates
  ...
  if ( (exportfits !=1) || (export != 1) ) { // Do the standard plotting
    ...
    else { sprintf(specmaptitle, "Injection Index by Region Map of %s", settarget); }
	...
	for (j=0; j<jsize; j++) {
	  if ( ((i+jstart-border) < 0) || ((j+istart-border) < 0) || ((i+jstart-border) >= xaxismax) || ((j+istart-border) >= yaxismax) ) {
	    ...
	    tmp_smootharray[i*jsize+j] = inj_regbestinject[regionarray[i+(jstart-border)][j+(istart-border)]];
	...
	adj_maxrange = 0.1;
      ...
      cpgwedg("RI", 1.5, 5.0, minrange, adj_maxrange, "Injection index");
	...
	for (j=jstart; j<jend; j++){
	  if (regionarray[i][j] <= 0) {
	    ...
	    switch ((int)inj_regbestinject[regionarray[i][j]]) {
	    case 0:
	      intenscale = 1;
	      break;
	    case 1:
	...
	for (j=0; j<yaxismax; j++) {
	  ...
	  if (regionarray[i][j] < 0) { // Set void regions to a high value to create a white background
	    ...
	    tmp_smootharray[i*xaxismax+j] = inj_regbestinject[regionarray[i][j]];
	...
	return 404;
      ...
      FunParamPuts(funconn, "BUNIT", 0,  "", "Units of injection index", 1);
	...
	strcpy(modelname, "Tribble (KP)");
      ...
      FunParamPuts(funconn, "HISTORY", 0, historytext, NULL, 1);
      strcpy(historytext, "BRATS  Injection index by region map made using the ");
